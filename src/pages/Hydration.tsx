...
import { Droplets, Plus, Minus, Coffee, GlassWater, Milk, Flame, Bell, Trophy, Activity, Target, Brain, History } from "lucide-react";
...
const Hydration = () => {
  const { profile, todayWater, weekWater, loading: dataLoading } = useUserData();
  const [user, setUser] = useState<User | null>(null);
  const [saving, setSaving] = useState(false);
  
  // intake is derived from todayWater if available, otherwise 0
  const intake = todayWater?.total_ml ?? 0;
  const goal = profile?.goals?.water ?? 3000;
  const pct = Math.min((intake / goal) * 100, 100);
  const fillHeight = pct;

  // Convert weekWater to the chart format
  const weekData = weekWater.map(d => ({
  ...
  const add = async (ml: number) => {
    ...
    setSaving(true);
    try {
      await logWater(user.uid, todayStr(), ml);
      toast.success(`Added ${ml}ml of water!`);
    } catch (e: any) {
      console.error(e);
      toast.error("Failed to save water intake: " + (e.message || "Unknown error"));
    } finally {
      setSaving(false);
    }
  };

  const remove = async (ml: number) => {
    if (intake <= 0 || !user || !db) return;
    setSaving(true);
    try {
      const actualRemoved = Math.min(ml, intake);
      await logWater(user.uid, todayStr(), -actualRemoved);
      toast.info(`Removed ${actualRemoved}ml of water.`);
    } catch (e: any) {
      console.error(e);
      toast.error("Failed to sync removal: " + (e.message || "Unknown error"));
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <DashboardLayout
      ...
      action={<Button variant="hero" size="sm" onClick={() => add(250)} disabled={saving}><Plus className="h-4 w-4" /> Add water</Button>}
    >
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <Card className="lg:col-span-1 p-6 bg-gradient-card border-border/50 relative overflow-hidden flex flex-col items-center">
          ...
          <div className="relative">
            ...
            <div className="font-display text-5xl font-bold text-center gradient-text mb-6 tabular-nums">
              {(intake / 1000).toFixed(2)}<span className="text-2xl text-muted-foreground">L</span>
            </div>
            ...
            <div className="relative w-48 h-72 mx-auto">
              <svg viewBox="0 0 200 300" className="w-full h-full">
                ...
                <path d="M70 40 L70 70 Q70 80 60 90 L50 110 Q40 130 40 160 L40 270 Q40 285 55 285 L145 285 Q160 285 160 270 L160 160 Q160 130 150 110 L140 90 Q130 80 130 70 L130 40 Z"
                  fill="hsl(var(--surface-1))" stroke="hsl(var(--border))" strokeWidth="2" />
                <g clipPath="url(#bottleClip)">
                  <rect x="0" y={300 - (260 * fillHeight) / 100} width="200" height="300"
                    fill="url(#waterGrad)" style={{ transition: "y 1s cubic-bezier(0.22, 1, 0.36, 1)" }} />
                  <path
                ...
                <text x="100" y="180" textAnchor="middle" fill="white" fontSize="24" fontWeight="700"
            ...
            <div className="flex justify-center gap-2 mt-6">
              <Button size="icon" variant="outline" onClick={() => remove(250)}><Minus className="h-4 w-4" /></Button>
              <Button variant="hero" onClick={() => add(250)} className="px-6" disabled={saving}>
                <Plus className="h-4 w-4" /> 250 ml
              </Button>
              <Button size="icon" variant="outline" onClick={() => add(500)}><Plus className="h-4 w-4" /></Button>
            </div>
            {saving && <p className="text-xs text-muted-foreground text-center mt-2">Saving...</p>}
          </div>
        </Card>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card className="p-6 bg-gradient-card border-border/50">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                ...
                <button key={q.label} onClick={() => add(q.ml)} disabled={saving}
                  className="group p-4 rounded-xl bg-surface-1 border border-border/40 hover:border-primary/40 hover:shadow-glow-sm transition-all text-center disabled:opacity-50">
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            {[
              { icon: Flame, label: "Today's intake", value: `${(intake / 1000).toFixed(2)}`, unit: "L", tone: "primary" },
              { icon: Droplets, label: "Goal", value: `${(goal / 1000).toFixed(1)}`, unit: "L", tone: "accent" },
              { icon: Bell, label: "Remaining", value: `${((goal - intake) / 1000).toFixed(2)}`, unit: "L", tone: "success" },
            ].map((s) => (
            ...
            {weekData.length > 0 ? (
              ...
              <div className="h-56 flex items-center justify-center">
                <div className="text-center space-y-2">
                  ...
                  <p className="text-sm text-muted-foreground">No water logged this week yet.</p>
                  <p className="text-xs text-muted-foreground">Add your intake above to see your history!</p>
                </div>
              </div>
            )}
          </Card>
